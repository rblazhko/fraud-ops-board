...
def _metrics_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "## Eval (future window)",
        f"- PR-AUC: **{ev['pr_auc']:.4f}**",
        f"- ROC-AUC: {ev['roc_auc']:.4f}",
        f"- Precision@50: {ev['precision_at_50']:.4f}",
        f"- Precision@100: {ev['precision_at_100']:.4f}",
        f"- Fraud rate: {ev['fraud_rate']:.4f} (n={ev['n']})",
        "",
        "## Train (reference)",
